module MatchesHelper
	...
	def process_selections
	    ...
	    if !user.nil?
	      ...
	      if return_call #this means that one of the visited or wants to visit match has been called
	        @matches = User.query_as(:users).match(user2: User).match("#{nonjoin_match_exp}").where("#{nonjoin_where_exp}").with(:user2, strength: 'count(user2)').order('strength DESC').proxy_as(User, :user2).paginate(:page => params[:page], per_page: 5)
	        non_default = true
	      elsif residence_select
	        @matches = User.query_as(:users).match("#{match_exp}").where("#{where_exp}").proxy_as(User, :users).paginate(:page => params[:page], per_page: 5, return: :'distinct users')
	        non_default = true
	      elsif gender_select
	        @matches = User.query_as(:users).match("#{match_exp}").where("#{where_exp}").proxy_as(User, :users).paginate(:page => params[:page], per_page: 5, return: :'distinct users')
	        non_default = true
	      ...
	      if !non_default
	        if age_select
	          @matches = User.as(:users).where("users.name <> '#{username}'").paginate(:page => params[:page], per_page: 5, return: :'distinct users')
	...
	def default_match 
	    ...
	    if !user.nil?
	      ...
	      @matches = User.query_as(:users).match(user2: User).match("#{match_exp}").where("#{where_exp}").with(:user2, strength: 'count(user2)').order('strength DESC').return(:user2).proxy_as(User, :user2).paginate(:page => params[:page], per_page: 5, return: :'distinct user2')
	...
	def default_back_up( current_results, cur_user )
		...
		if current_results.length < 10
		  default = User.query_as(:users).where("#{where_exp}").proxy_as(User, :users).paginate(:page => params[:page], per_page: 5, return: :'distinct users')
		  return default
		else
		  where_exp << "AND users.uuid IN #{id_list}"
		  join_match = User.query_as(:users).where("#{where_exp}").proxy_as(User, :users).paginate(:page => params[:page], per_page: 5, return: :'distinct users')
		  return join_match
		end
	end
